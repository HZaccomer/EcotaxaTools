#' compute_bv
#'
#' Compute biovolumes for all the samples and save the new tsv files.
#'
...
compute_bv <- function(path, output, metadata=NULL) {
  ...
  # # Trophic levels
  # if(file.exists(""))
  # groupes <- data_bis %>% mutate(object_annotation_category2=
  #                                  object_annotation_hierarchy %>%
  #                                  gsub("-","_",.) %>% gsub(">","_",.) %>% gsub("<","_",.)) %>%
  #   merge(read_csv2("data/zooregroup_zooscan.csv"), all=T) %>%
  #   group_by(Value, ID, object_date, intercalibration) %>% summarise(BV=sum(BV_elli * conver, na.rm=T)) %>%
  #   filter(!is.na(Value), Value>=0, Value <=3)
  #
  # groupes$categorie[groupes$Value==1] <- "Phototrophs"
  # groupes$categorie[groupes$Value==1.5] <- "Mixotrophs"
  # groupes$categorie[groupes$Value==2] <- "Grazers"
  # groupes$categorie[groupes$Value==2.5] <- "Omnivorous"
  # groupes$categorie[groupes$Value==3] <- "Predators"
  #
  # groupes <- arrange(groupes, Value) %>% group_by(ID, object_date) %>%
